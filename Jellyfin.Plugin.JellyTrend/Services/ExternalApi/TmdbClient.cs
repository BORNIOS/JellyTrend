...
namespace Jellyfin.Plugin.JellyTrend.Services.ExternalApi;
...
public sealed class TmdbClient
{
    ...
    /// <param name="logger">Instance of the <see cref="ILogger{TmdbClient}"/> interface.</param>
    ...
    /// <summary>
    /// Fetches the trending movies from TMDB.
    /// </summary>
    /// <param name="apiKey">The TMDB API key.</param>
    /// <param name="maxItems">The maximum number of items to return.</param>
    /// <param name="language">The BCP-47 language tag.</param>
    /// <param name="region">The ISO 3166-1 alpha-2 region code.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The list of trending movies.</returns>
    public Task<IReadOnlyList<TmdbItem>> GetTrendingMoviesAsync(
    ...
    /// <summary>
    /// Fetches the trending TV shows from TMDB.
    /// </summary>
    /// <param name="apiKey">The TMDB API key.</param>
    /// <param name="maxItems">The maximum number of items to return.</param>
    /// <param name="language">The BCP-47 language tag.</param>
    /// <param name="region">The ISO 3166-1 alpha-2 region code.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The list of trending TV shows.</returns>
    public Task<IReadOnlyList<TmdbItem>> GetTrendingTvAsync(
        string apiKey, int maxItems, string language, string region, CancellationToken cancellationToken)
        => FetchTrendingAsync("tv", apiKey, maxItems, language, region, cancellationToken);

    private async Task<IReadOnlyList<TmdbItem>> FetchTrendingAsync(
        string mediaType,
        string apiKey,
        int maxItems,
        string language,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            extras += $"&language={Uri.EscapeDataString(language)}";
        }

        if (!string.IsNullOrWhiteSpace(region))
        {
            extras += $"&region={Uri.EscapeDataString(region)}";
        }
